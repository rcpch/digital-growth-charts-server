    ...
    def validate_observation_value(cls, values):
        ...
        if measurement_method == "bmi":
            if calculated_sds < MINIMUM_BMI_ERROR_SDS:
                raise ValueError("BMI cannot be less than -15 SD.")
            if calculated_sds > MAXIMUM_BMI_ERROR_SDS:
                raise ValueError("BMI cannot be more than +15 SD.")
        else:
            if calculated_sds < MINIMUM_HEIGHT_WEIGHT_OFC_ERROR_SDS:
                raise ValueError(f"{values.measurement_method} cannot be less than -8 SD.")
            if calculated_sds > MAXIMUM_HEIGHT_WEIGHT_OFC_ERROR_SDS:
                raise ValueError(f"{values.measurement_method} cannot be more than +8 SD.")
        return values
