...
@who.post("/calculation", tags=["who"], response_model=MeasurementObject)
def who_calculation(
    ...
    """
    ## WHO Centile and SDS Calculations

    * These are the 'standard' centiles for children in North America.
    ...
    * Optional events can be passed in as a list of strings - each list is associated with a measurement
    ...
    try:
        validate_observation_value(WHO, measurementRequest)
...
def who_chart_coordinates(chartParams: ChartCoordinateRequest):
    """
    ## WHO Chart Coordinates data.
    ...
    * If no centile_format is supplied, "cole-nine-centiles" are returned as a default.
    ...
    if type(chartParams.centile_format) is list:
        ...
        chart_data_file = Path(
            f"chart-data/{chartParams.centile_format}-{constants.WHO}-{chartParams.sex}-{chartParams.measurement_method}.json"
        )
        if chart_data_file.exists():
            print(
                f"Chart data file exists for {chartParams.centile_format}-{constants.WHO}-{chartParams.sex}-{chartParams.measurement_method}."
            )
            with open(
                f"chart-data/{chartParams.centile_format}-{constants.WHO}-{chartParams.sex}-{chartParams.measurement_method}.json",
                "r",
...
@who.post(
    "/fictional-child-data", tags=["who"], response_model=List[MeasurementObject]
)
def fictional_child_data(fictional_child_request: FictionalChildRequest):
    """
    ## WHO Fictional Child Data Endpoint
