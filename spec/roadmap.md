...
- [ ] **R11 - Replace checked-in generated chart coordinates with an explicit cache strategy**
  ...
  Planning is recorded in [Chart-coordinate lifecycle](chart-coordinate-lifecycle.md). The preferred direction is to generate the full coordinate set once per running server process, held in memory only - no disk persistence, since there's no measurable advantage to writing it to disk and a real risk of a process silently serving coordinates from a stale engine version. React keeps its bundled curves and avoids additional billable API calls; the leading proposal there is a CI build step in the Component repository that regenerates the bundle from a pinned `rcpchgrowth` release, rather than a hand-maintained or manually-regenerated dataset. Before implementation, preserve and compare historical output with fresh generation, review discrepancies for clinical significance, benchmark startup and representative API operations, and agree provenance and readiness/failure contracts. Acceptance requires reproducible generation, freshness tests, independently reviewed coordinate expectations, coordinate-specific React rendering checks, the API regression suite, and all supported compatibility profiles to pass. Runtime implementation remains outstanding.
