...
class Measurement:
    ...
    def __calculate_weight_sds_centile(self, sex: str, age: float, weight: float, default_to_youngest_reference: bool=False, born_preterm: bool = False):
        if weight and weight > 0.0:
            weight_sds = sds(age=age, measurement='weight', measurement_value=weight, sex=sex, default_to_youngest_reference=False, born_preterm=born_preterm)
    ...
    def __calculate_bmi_sds_centile(self, sex: str, age: float, born_preterm: bool = False, default_to_youngest_reference: bool=False, height: float = 0.0, weight: float = 0.0, bmi: float = 0.0):
        ...
        if (height and height > 0.0) and (weight and weight > 0.0): 
            ...
            if age > FORTY_TWO_WEEKS_GESTATION: # BMI data not present < 42 weeks gestation
                bmi_sds = sds(age=age, measurement='bmi', measurement_value=bmi, sex=sex, default_to_youngest_reference=default_to_youngest_reference, born_preterm=born_preterm) ## does not default to youngest reference
    ...
    def __calculate_ofc_sds_centile(self, sex: str, age: float, ofc: float, default_to_youngest_reference: bool=False, born_preterm: bool = False):
        if ofc and ofc > 0.0:
            if (age <= 17 and sex == 'female') or (age <= 18.0 and sex == 'male'): # OFC data not present >17y in girls or >18y in boys
                ofc_sds = sds(age=age, measurement='ofc', measurement_value=ofc, sex=sex, default_to_youngest_reference=default_to_youngest_reference, born_preterm=born_preterm)
