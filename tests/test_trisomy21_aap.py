...
def test_trisomy_21_aap_calculation_with_valid_request():
    ...
    response = client.post("/trisomy-21-aap/calculation", json=body)

    # load the known-correct response from file
    with open(r"tests/test_data/test_trisomy_21_aap_calculation_valid.json", "r") as file:
...
def test_trisomy_21_app_chart_with_valid_request_for_outside_range_values(input):
...
def test_trisomy_21_aap_chart_data_with_valid_request(input):
    body = input | {
        "centile_format": "five-percent-centiles",
    }

    response = client.post("/trisomy-21-aap/chart-coordinates", json=body)
...
def test_trisomy_21_aap_chart_data_with_invalid_request():
...
def test_trisomy_21_aap_fictional_child_data_with_valid_request():
    ...
    response = client.post("/trisomy-21-aap/fictional-child-data", json=body)
...
def test_trisomy_21_aap_bulk_calculation_all_valid():
    ...
    response = client.post("/trisomy-21-aap/bulk-calculation", json=body)
    assert response.status_code == 200
    data = response.json()
...
def test_trisomy_21_aap_bulk_calculation_partially_valid():
    body = {
        "measurement_method": "height",
        "birth_date": "2020-04-12",
        "sex": "female",
...
def test_trisomy_21_aap_bulk_calculation_all_invalid():
    body = {
        "measurement_method": "height",
        "birth_date": "2020-04-12",
        "sex": "female",
...
def test_trisomy_21_aap_fictional_child_data_with_invalid_request():
