...
def test_ukwho_bulk_calculation_all_valid():
    ...
    response = client.post("/uk-who/bulk-calculation", json=body)
    assert response.status_code == 200
    data = response.json()
...
def test_ukwho_bulk_calculation_partially_valid():
    body = {
        "measurement_method": "height",
        "birth_date": "2020-04-12",
        "sex": "female",
    ...
    # one should be a calculation object, one an error dict with msg
    assert any("measurement_calculated_values" in r for r in results)
    assert any("msg" in r for r in results)


def test_ukwho_bulk_calculation_all_invalid():
    body = {
        "measurement_method": "height",
        "birth_date": "2020-04-12",
        "sex": "female",
